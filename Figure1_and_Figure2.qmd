...
knitr::opts_knit$set(root.dir = '~/OneDrive/Studium/MSc_CBB/Semester_1_2024_HS/STA426/project/DA_COMPARE_DATA_21_03_08/Hackathon/Studies/')
setwd("~/OneDrive/Studium/MSc_CBB/Semester_1_2024_HS/STA426/project/DA_COMPARE_DATA_21_03_08/Hackathon/Studies/")
...
tool_names <- c(aldex2="ALDEx2", 
                ancom="ANCOM-II", 
                corncob="corncob", 
                deseq2="DESeq2", 
                edger="edgeR", 
                lefse="LEfSe", 
                limma_voom_TMM="limma voom (TMM)", 
                limma_voom_TMMwsp="limma voom (TMMwsp)", 
                maaslin2="MaAsLin2",
                maaslin2rare="MaAsLin2 (rare)",
                metagenomeSeq="metagenomeSeq",
                ttestrare="t-test (rare)", 
                wilcoxonclr="Wilcoxon (CLR)", 
                wilcoxonrare="Wilcoxon (rare)")

Metadata_renames <- c(log_N="log(Sample size)", 
                      R.squared="Aitchson's Dist. Effect Size", 
                      log_R.squared="log(Aitch. dist. effect size)", 
                      Sparsity="Sparsity", 
                      Richness="Richness",
                      log_Depth="log(Max read depth)", 
                      log_Depth_range="log(Read depth range)", 
                      CoV_Depth="Read depth variation", 
                      filtered_out_percent = "% below prev. cut-off")
...
# Check if all Tool Ran on all Studies
```{r}
# Function to create the presence table
generate_presence_table <- function(results_type, hackathon_study_ids, da_tools_out) {
  # Initialize an empty data frame
  presence_table <- matrix(FALSE, nrow = length(hackathon_study_ids), ncol = length(da_tools_out))
  rownames(presence_table) <- hackathon_study_ids
  colnames(presence_table) <- da_tools_out
  
  # Loop through each study and tool to check for any .tsv file
  for (study in hackathon_study_ids) {
    for (tool in da_tools_out) {
      # Construct the directory path
      tool_dir <- file.path(".", study, results_type, tool)
      
      # Check if the tool directory exists
      if (dir.exists(tool_dir)) {
        # Now check if a .tsv file exists in the directory or its subdirectories
        tsv_files <- list.files(tool_dir, pattern = "\\.tsv$", full.names = TRUE, recursive = TRUE)
        if (length(tsv_files) >= 1) {
          presence_table[study, tool] <- TRUE
        }
      } else {
        print(paste("Directory does not exist:", tool_dir))
      }
    }
  }
...
fix_results_table <- generate_presence_table("Fix_Results_0.1", hackathon_study_ids, da_tools_out)
no_filt_results_table <- generate_presence_table("No_filt_Results", hackathon_study_ids, da_tools_out)

#Directory does not exist: ./seston_plastic_mccormick
#Directory does not exist: ./sw_plastic_frere
#no study ran with Lefse_out
```
...
filt_results <- lapply(hackathon_study_ids, read_hackathon_results, results_folder = "Fix_Results_0.1")
